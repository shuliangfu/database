...
const TABLE_SOFT_DELETE = "mysql_model_soft_delete_users";
const TABLE_CATEGORIES = "mysql_model_categories";
...
describe("SQLModel MySQL", () => {
  ...
  beforeAll(async () => {
    ...
    // 创建测试表
    await adapter.execute(`DROP TABLE IF EXISTS ${TABLE_NAME}`, []).catch(
      () => {},
    );
    ...
    // 创建软删除测试表
    await adapter.execute(
      `DROP TABLE IF EXISTS ${TABLE_SOFT_DELETE}`,
      [],
    ).catch(() => {});
    await adapter.execute(
