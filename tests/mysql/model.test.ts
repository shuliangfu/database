...
class User extends SQLModel {
  static override tableName = "mysql_users";
...
describe("SQLModel", () => {
  ...
  beforeAll(async () => {
    ...
    // 删除旧表（如果存在）并创建新表，确保表结构正确
    // 注意：使用 MySQL/MariaDB 语法
    await adapter.execute("DROP TABLE IF EXISTS users", []).catch(() => {});
  ...
  describe("init", () => {
    it("应该初始化模型", async () => {
      class TestModel extends SQLModel {
        static override tableName = "mysql_test_table";
      }
  ...
  describe("find", () => {
    beforeEach(async () => {
      // 清理并插入测试数据
      await adapter.execute("DELETE FROM mysql_users", []);
  ...
  describe("paginate", () => {
    ...
    it("应该支持字段投影", async () => {
      ...
      expect("name" in firstItem).toBe(true);
      expect("email" in firstItem).toBe(true);
      // 注意：age 不在投影中，所以不应该比较 age
      // 验证数据存在即可
      expect(firstItem.name).toBeTruthy();
      expect(firstItem.email).toBeTruthy();
    });
  ...
  describe("数据验证", () => {
    ...
    it("应该验证存在性（exists）", async () => {
      // 先创建一个分类表
      await adapter.execute(
        "CREATE TABLE IF NOT EXISTS categories (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255))",
        [],
      );
      await adapter.execute(
      ...
      class Category extends SQLModel {
        static override tableName = "mysql_categories";
        static override primaryKey = "id";
        static schema: ModelSchema = {
          name: { type: "string" },
        };
    ...
    it("应该验证不存在性（notExists）", async () => {
      ...
      // 创建被禁止邮箱表
      await adapter.execute(
        "CREATE TABLE IF NOT EXISTS banned_emails (id INT AUTO_INCREMENT PRIMARY KEY, email VARCHAR(255))",
        [],
      );
      await adapter.execute(
  ...
  describe("withTrashed", () => {
    beforeEach(async () => {
      // 在每个测试前清理软删除测试表的数据
      await adapter.execute("TRUNCATE TABLE soft_delete_users", []).catch(
        () => {
          // 如果表不存在，忽略错误
        },
      );
    ...
    it("应该包含已删除记录的查询（静态方法）", async () => {
      ...
      class SoftDeleteUser extends SQLModel {
        static override tableName = "mysql_soft_delete_users";
        static override primaryKey = "id";
        static override softDelete = true;
        static override deletedAtField = "deleted_at";
      }
