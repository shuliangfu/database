/**
 * @fileoverview SQLModel SQLite 测试 - 基于最新统一接口
 */
...
// 定义表名常量（注意：SQLite 不允许使用 sqlite_ 开头的表名，这是保留命名空间）
const TABLE_NAME = "test_sqlite_users";
const TABLE_SOFT_DELETE = "test_sqlite_soft_delete_users";
const TABLE_CATEGORIES = "test_sqlite_categories";
...
describe("SQLModel SQLite", () => {
  let adapter: DatabaseAdapter;

  beforeAll(async () => {
    // 获取 SQLite 连接配置
    const sqlitePath = getEnvWithDefault(
      "SQLITE_PATH",
      ":memory:",
    );
  ...
  describe("关联方法", () => {
    const TABLE_POSTS = "test_sqlite_posts";
    const TABLE_PROFILES = "test_sqlite_profiles";
