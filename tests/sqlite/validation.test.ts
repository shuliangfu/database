...
const TABLE_NAME = "test_sqlite_validation_users";
const TABLE_CATEGORIES = "test_sqlite_validation_categories";
const TABLE_BANNED_EMAILS = "test_sqlite_validation_banned_emails";
const TABLE_UNIQUE_TEST = "test_sqlite_validation_unique_test";
...
describe("SQLModel 数据验证", () => {
  ...
  beforeAll(async () => {
    try {
      ...
      // 创建其他测试表
      await adapter.execute(`DROP TABLE IF EXISTS ${TABLE_CATEGORIES}`, [])
        .catch(() => {});
      await adapter.execute(
      ...
      await adapter.execute(`DROP TABLE IF EXISTS ${TABLE_BANNED_EMAILS}`, [])
        .catch(() => {});
      await adapter.execute(
      ...
      await adapter.execute(`DROP TABLE IF EXISTS ${TABLE_UNIQUE_TEST}`, [])
        .catch(() => {});
      await adapter.execute(
      ...
      console.log("SQLite not available, skipping validation tests");
      console.error(error);
    }
  });
