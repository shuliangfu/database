...
const TABLE_SOFT_DELETE = "postgresql_model_soft_delete_users";
const TABLE_CATEGORIES = "postgresql_model_categories";
...
describe("SQLModel PostgreSQL", () => {
  ...
  beforeAll(async () => {
    ...
    // 创建软删除测试表
    await adapter.execute(
      `DROP TABLE IF EXISTS ${TABLE_SOFT_DELETE} CASCADE`,
      [],
    ).catch(() => {});
    await adapter.execute(
      `CREATE TABLE ${TABLE_SOFT_DELETE} (
        id SERIAL PRIMARY KEY,
        name VARCHAR(255) NOT NULL,
        email VARCHAR(255) UNIQUE NOT NULL,
        deleted_at TIMESTAMP
  ...
  describe("update", () => {
    ...
    it("应该更新记录（通过 ID）", async () => {
      ...
      const affected = await User.update(user!.id, { name: "Updated Name" });
  ...
  describe("updateById", () => {
    ...
    it("应该通过 ID 更新记录", async () => {
      ...
      const affected = await User.updateById(user!.id, {
        name: "UpdatedById",
      });
      expect(affected).toBe(1);
  ...
  describe("increment", () => {
    beforeEach(async () => {
      await adapter.execute(`DELETE FROM ${TABLE_NAME}`, []);
      await adapter.execute(
        `ALTER TABLE ${TABLE_NAME} ADD COLUMN IF NOT EXISTS score INTEGER DEFAULT 0`,
        [],
      ).catch(() => {});
      await User.create({
        name: "Increment Test",
    ...
    it("应该增加字段值", async () => {
      ...
      const result = await User.increment(user!.id, "age", 5);
    ...
    it("应该支持对象格式（批量自增）", async () => {
      const user = await User.findOne({ email: "increment@test.com" });
      const result = await User.increment(user!.id, { age: 5, score: 10 });
    ...
    it("应该支持 returnLatest 选项", async () => {
      const user = await User.findOne({ email: "increment@test.com" });
      const result = await User.increment(user!.id, "age", 5, true);
      expect(typeof result).toBe("object");
      expect((result as any).age).toBe(30);
    });
    ...
    it("应该支持对象格式的 returnLatest 选项", async () => {
      ...
      expect(typeof result).toBe("object");
      expect((result as any).age).toBe(28);
      expect((result as any).score).toBe(15);
    });
  });
  ...
  describe("decrement", () => {
    ...
    it("应该减少字段值", async () => {
      ...
      const result = await User.decrement(user!.id, "age", 5);
      expect(typeof result).toBe("number");
    ...
    it("应该支持对象格式（批量自减）", async () => {
      const user = await User.findOne({ email: "decrement@test.com" });
      const result = await User.decrement(user!.id, { age: 5, score: 10 });
      expect(result).toBe(1);

      const updated = await User.find(user!.id);
      expect(updated?.age).toBe(25);
      expect((updated as any)?.score).toBe(10);
    });

    it("应该支持 returnLatest 选项", async () => {
      const user = await User.findOne({ email: "decrement@test.com" });
      const result = await User.decrement(user!.id, "age", 5, true);
      expect(typeof result).toBe("object");
    ...
    it("应该支持对象格式的 returnLatest 选项", async () => {
      ...
      expect(typeof result).toBe("object");
      expect((result as any).age).toBe(27);
      expect((result as any).score).toBe(15);
    });
  });
  ...
  describe("incrementMany", () => {
    beforeEach(async () => {
      await adapter.execute(`DELETE FROM ${TABLE_NAME}`, []);
      await adapter.execute(
        `ALTER TABLE ${TABLE_NAME} ADD COLUMN IF NOT EXISTS score INT DEFAULT 0`,
        [],
      ).catch(() => {});
    });
  ...
  describe("upsert", () => {
    ...
    it("应该支持 resurrect 参数", async () => {
      class SoftDeleteUser extends SQLModel {
        static override tableName = TABLE_SOFT_DELETE;
      ...
      expect((updated as any).deleted_at).toBeNull();
    });
  });
  ...
  describe("findOrCreate", () => {
    ...
    it("应该支持 resurrect 参数", async () => {
      ...
      expect((found as any).deleted_at).toBeNull();
    });
  });
  ...
  describe("findOneAndDelete", () => {
    ...
    it("应该查找并删除记录", async () => {
      ...
      const deleted = await User.findOneAndDelete({ id: user.id });
  ...
  describe("软删除相关方法", () => {
    beforeEach(async () => {
      await adapter.execute(`DELETE FROM ${TABLE_SOFT_DELETE}`, []);
    });
    ...
    it("应该支持 restoreById 通过 ID 恢复", async () => {
      ...
      const restored = await SoftDeleteUser.restoreById(user.id);
    ...
    it("应该支持 forceDeleteById 通过 ID 强制删除", async () => {
      ...
      const affected = await SoftDeleteUser.forceDeleteById(user.id);
      expect(affected).toBe(1);
  ...
  describe("query 链式查询构建器", () => {
    ...
    it("应该支持链式查询 increment 的对象格式", async () => {
      await adapter.execute(
        `ALTER TABLE ${TABLE_NAME} ADD COLUMN IF NOT EXISTS score INTEGER DEFAULT 0`,
        [],
      ).catch(() => {});
      ...
      const user = await User.create({
        name: "QueryIncrementObject",
        email: "queryincrementobject@test.com",
        age: 25,
        score: 10,
      });
    ...
    it("应该支持链式查询 increment 对象格式的 returnLatest 参数", async () => {
      await adapter.execute(
        `ALTER TABLE ${TABLE_NAME} ADD COLUMN IF NOT EXISTS score INTEGER DEFAULT 0`,
        [],
      ).catch(() => {});
      ...
      const user = await User.create({
        name: "QueryIncrementObjectReturnLatest",
        email: "queryincrementobjectreturnlatest@test.com",
        age: 25,
        score: 10,
      });
      ...
      const updated = await (User.query()
        .where({ id: user.id }) as any)
        .increment({ age: 3, score: 5 }, true);
      expect(typeof updated).toBe("object");
      expect((updated as any).age).toBe(28);
      expect((updated as any).score).toBe(15);
    });
    ...
    it("应该支持链式查询 decrement 的对象格式", async () => {
      await adapter.execute(
        `ALTER TABLE ${TABLE_NAME} ADD COLUMN IF NOT EXISTS score INTEGER DEFAULT 0`,
        [],
      ).catch(() => {});
      ...
      const user = await User.create({
        name: "QueryDecrementObject",
        email: "querydecrementobject@test.com",
        age: 30,
        score: 20,
      });
    ...
    it("应该支持链式查询 decrement 对象格式的 returnLatest 参数", async () => {
      await adapter.execute(
        `ALTER TABLE ${TABLE_NAME} ADD COLUMN IF NOT EXISTS score INTEGER DEFAULT 0`,
        [],
      ).catch(() => {});
      ...
      const user = await User.create({
        name: "QueryDecrementObjectReturnLatest",
        email: "querydecrementobjectreturnlatest@test.com",
        age: 30,
        score: 20,
      });
      ...
      const updated = await (User.query()
        .where({ id: user.id }) as any)
        .decrement({ age: 3, score: 5 }, true);
      expect(typeof updated).toBe("object");
      expect((updated as any).age).toBe(27);
      expect((updated as any).score).toBe(15);
    });
    ...
    it("应该支持链式查询 updateById", async () => {
      ...
      const affected = await User.query()
        .updateById(user.id, { name: "UpdatedById" });
      expect(affected).toBe(1);
    ...
    it("应该支持链式查询 restore", async () => {
      ...
      const found1 = await SoftDeleteUser.find(user1.id);
      const found2 = await SoftDeleteUser.find(user2.id);
  ...
  describe("scope", () => {
    ...
    it("应该支持作用域查询", async () => {
      class ScopedUser extends SQLModel {
        static override tableName = TABLE_NAME;
  ...
  describe("关联方法", () => {
    const TABLE_POSTS = "postgresql_model_posts";
    const TABLE_PROFILES = "postgresql_model_profiles";

    beforeAll(async () => {
      // 创建关联测试表
      await adapter.execute(
        `DROP TABLE IF EXISTS ${TABLE_POSTS} CASCADE`,
        [],
      ).catch(() => {});
      await adapter.execute(
      ...
      await adapter.execute(
        `DROP TABLE IF EXISTS ${TABLE_PROFILES} CASCADE`,
        [],
      ).catch(() => {});
      await adapter.execute(
    ...
    beforeEach(async () => {
      await adapter.execute(`DELETE FROM ${TABLE_NAME}`, []);
      await adapter.execute(`DELETE FROM ${TABLE_POSTS}`, []);
      await adapter.execute(`DELETE FROM ${TABLE_PROFILES}`, []);
    });

    it("应该支持 belongsTo 关联查询", async () => {
      class Post extends SQLModel {
        static override tableName = TABLE_POSTS;
        static override primaryKey = "id";
      }
      ...
      const relatedUser = await post.belongsTo(User, "user_id", "id");
    ...
    it("应该支持 belongsTo 的 includeTrashed 参数", async () => {
      ...
      expect(relatedUser2?.id).toBe(user.id);
    });

    it("应该支持 hasOne 关联查询", async () => {
      class Profile extends SQLModel {
        static override tableName = TABLE_PROFILES;
        static override primaryKey = "id";
      }
      ...
      const relatedProfile = await user.hasOne(Profile, "user_id", "id");
      expect(relatedProfile).toBeTruthy();
      expect(relatedProfile?.id).toBe(profile.id);
    ...
    it("应该支持 hasOne 的 includeTrashed 参数", async () => {
      class SoftDeleteProfile extends SQLModel {
        static override tableName = TABLE_PROFILES;
        static override softDelete = true;
        static override deletedAtField = "deleted_at";
      }
      ...
      await SoftDeleteProfile.delete(profile.id);
      ...
      expect(relatedProfile2?.id).toBe(profile.id);
    });
    ...
    it("应该支持 hasMany 的 fields 参数", async () => {
      ...
      const posts = await user.hasMany(Post, "user_id", "id", ["title"]);
    ...
    it("应该支持 hasMany 的 includeTrashed 参数", async () => {
      class SoftDeletePost extends SQLModel {
        static override tableName = TABLE_POSTS;
        static override softDelete = true;
        static override deletedAtField = "deleted_at";
      }
      ...
      await SoftDeletePost.delete(post1.id);

      // 默认不包含软删除记录
      const posts1 = await user.hasMany(SoftDeletePost, "user_id", "id");
      expect(posts1.length).toBe(1);
      expect(posts1[0].id).toBe(post2.id);
  ...
  // PostgreSQL 客户端库可能有内部定时器和资源，禁用泄漏检查
  sanitizeOps: false,
