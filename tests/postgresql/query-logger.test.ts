...
describe("QueryLogger", () => {
  ...
  describe("log", () => {
    ...
    it("应该记录执行日志", () => {
      ...
      expect(logs[0].sql).toBe("INSERT INTO users (name) VALUES ($1)");
    ...
    it("应该在 logLevel 为 slow 时只记录慢查询", () => {
      ...
      logger.log("query", "SELECT * FROM users", [], 50);
      logger.log("query", "SELECT * FROM posts", [], 150);
      logger.log("query", "SELECT * FROM comments", [], 200);
    ...
    it("应该在 logLevel 为 slow 时记录错误查询", () => {
      ...
      const error = new Error("Database error");
      logger.log("query", "SELECT * FROM users", [], 50, error);
  ...
  describe("翻译函数 (t)", () => {
    ...
    it("不传 lang 时按环境检测语言，仍能打出含 sql 的日志", () => {
      const calls: { method: string; msg: string }[] = [];
      const mockLogger = {
        debug: (msg: string) => calls.push({ method: "debug", msg }),
        info: (msg: string) => calls.push({ method: "info", msg }),
    ...
    it("慢查询时打出 warn 且消息含 sql 与 duration", () => {
      const calls: { method: string; msg: string }[] = [];
      const mockLogger = {
        debug: (msg: string) => calls.push({ method: "debug", msg }),
        info: (msg: string) => calls.push({ method: "info", msg }),
      ...
      expect(calls[0].msg).toContain("SELECT * FROM big_table");
      expect(calls[0].msg).toContain("100");
    });
  });
