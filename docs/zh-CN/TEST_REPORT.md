...
- **测试库版本**：@dreamer/test@^1.0.11
- **运行时适配器**：@dreamer/runtime-adapter@^1.0.15
...
| 适配器                  | 用例数 | 说明               |
| ----------------------- | ------ | ------------------ |
| integration（多适配器） | 5      | 多库联合测试       |
| MongoDB                 | 523    | 文档库（含时区 6） |
| MySQL                   | 501    | 关系型             |
| PostgreSQL              | 508    | 关系型             |
| SQLite                  | 503    | 嵌入式             |
...
- **集成**：多适配器同时操作（MySQL、SQLite、MongoDB）5 个用例通过
- **MongoDB 时区**：mongoOptions.timezone 配置后查询结果 date
  自动格式化为指定时区字符串（getTimezone、Asia/Shanghai、PRC 等价）6 个用例通过
...
@dreamer/database 共 2,040 个用例全部通过，通过率 100%。
...
_测试报告生成日期：2026-02-19_ _测试工具：@dreamer/test（兼容 Deno 与 Bun）_ _含
Mongo 时区测试（timezone.test.ts，6 用例）_
